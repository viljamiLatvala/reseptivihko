...
def validate_password(min=-1, max=-1):
    def _validate_password(form, field):
        ...
        if len(password) < min or max != -1 and len(password) > max:
            raise ValidationError('Password must be between %d and %d characters long.' % (min, max)) 
            
        if password == password.upper() or password == password.lower():
            raise ValidationError('Password must contain both upper case and lower case letters.')
...
class SignupForm(FlaskForm):
    ...
    password_control = PasswordField("Re-type password",[validate_password_control])
